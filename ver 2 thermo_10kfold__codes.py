...
plt.plot(fpr_indep, tpr_indep, color='darkorange', lw=2, label='ROC curve (10-fold SVM AUC = %0.3f)' % roc_auc_indep)
...
plt.figure(figsize=(8, 6))
plt.plot(fpr_test, tpr_test, color='darkorange', lw=2, label='ROC curve (10-fold RF area = %0.3f)' % roc_auc_test)
plt.plot([0, 1], [0, 1], color='navy', lw=2, linestyle='--')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('False Positive Rate')
...
# Plot ROC Curve for test set
plt.figure(figsize=(8, 6))
plt.plot(etc_fpr_test, etc_tpr_test, color='darkorange', lw=2, label='10-fold Extra Tree ROC curve (area = %0.3f)' % etc_roc_auc_test)
plt.plot([0, 1], [0, 1], color='navy', lw=2, linestyle='--')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('False Positive Rate')
...
for fold, (train_index, test_index) in enumerate(kf.split(X, Y), 1):
    ...
    CNN1D.add(Flatten())
    CNN1D.add(Dense(256, activation='relu'))
    ...
    # Train model
    CNN1D.fit(X_train, Y_train, epochs=20, batch_size=16, verbose=1)
...
# Plot ROC Curve
plt.figure(figsize=(8, 8))
plt.plot(mlp_fpr, mlp_tpr, color='blue', lw=2, label='ROC curve (10 Fold MLP AUC = %0.3f)' % mlp_auc)
